...
export default function SwapPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [balanceData, poolData, lpData, priceData] = await Promise.all([
        address ? getAllBalances(address, config.explorerApiUrl, config.provableApiUrl) : null,
        getPoolInfo(),
  ...
  }, [address, config.explorerApiUrl, config.provableApiUrl, getPoolInfo, getLpPosition, getSpotPrice]);
  ...
  const setMaxAmount = () => {
    if (!balances) return;
    const balance = isPulseToUsdc ? balances[COIN_TYPES.PULSE] : balances[COIN_TYPES.STABLE];
    const decimals = isPulseToUsdc ? getCoinDecimals(COIN_TYPES.PULSE) : getCoinDecimals(COIN_TYPES.STABLE);
    const maxAmount = Number(balance.balance) / Math.pow(10, decimals);
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    ...
                    onClick={() => {
                      if (balances) {
                        const max = Number(balances[COIN_TYPES.PULSE].balance) / Math.pow(10, 8);
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    ...
                    onClick={() => {
                      if (balances) {
                        const max = Number(balances[COIN_TYPES.STABLE].balance) / Math.pow(10, 6);
