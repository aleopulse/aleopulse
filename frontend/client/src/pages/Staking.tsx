...
export default function StakingPage() {
  ...
  // Get PULSE balance (convert bigint to number for arithmetic)
  const pulseBalance = Number(balances?.[COIN_TYPES.PULSE]?.balance ?? 0n);
  ...
  const handleUnstake = async (positionIndex: number) => {
    try {
      ...
      // TODO: Aleo staking uses receipts - for now use position index as identifier
      const receipt = `position_${positionIndex}`;
